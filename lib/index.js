...
/**
 * Exports data as JSON (*.json)
 * @method
 * @param  {object} sortedModules     The sorted data from all packages as object.
 * @param  {object} customFormat      The custom format with information about the needed keys.
 * @return {string}                   The returning JSON string.
 */
exports.asJson = function(sortedModules, customFormat) {
    var output;

    if (customFormat && Object.keys(customFormat).length > 0) {

        output = {};
        var moduleKey, module, moduleName, moduleProp;

        for (moduleKey in sortedModules) {
            if (sortedModules.hasOwnProperty(moduleKey)) {
                module = sortedModules[moduleKey];
                moduleName = sortedModules[moduleKey].name;

                // Create property in output object
                output[moduleName] = {};

                for (moduleProp in module) {
                    if (module.hasOwnProperty(moduleProp)) {
                        if (moduleProp === 'name') continue;
                        // If property also exist in custom format, allow property
                        if (customFormat.hasOwnProperty(moduleProp)) {
                            output[moduleName][moduleProp] = sortedModules[moduleKey][moduleProp];
                        }
                    }
                }
            }
        ...
        output = JSON.stringify(sortedModules, null, 2);
    }
